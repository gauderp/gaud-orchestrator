...
export function ExecutionPRs({ tasks }: Props) {
  ...
  if (withPRs.length === 0) {
    return <p className="text-sm text-[--color-muted] dark:text-[--color-muted-dark]">No PRs created yet.</p>
  }
      ...
      {withPRs.map((task) => (
        ...
        >
          <div className="flex items-center gap-2">
            <span className="text-sm text-[--color-ink] dark:text-[--color-ink-dark]">{task.title}</span>
